...
export KERNELDIR="/workspace/Rsyd58/krnl"
export USE_CCACHE=1
...
export DEFCONFIG="vince-perf_defconfig"
...
export LOCALVERSION="-msm8953-arm64-Android-10-LA.UM.8.6.2.c26"
export KSU=y
make -j$(nproc --all) O=out LD=ld.lld ARCH=arm64  CC=clang HOSTCC=clang HOSTCXX=clang++ READELF=llvm-readelf HOSTAR=llvm-ar AR=llvm-ar AS=llvm-as NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi-  2>&1 | tee log.txt
